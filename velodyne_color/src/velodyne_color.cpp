...
public:
	...
	velodyne_lidar_color(ros::NodeHandle nh):n(nh)
	{

		subCloud=n.subscribe<sensor_msgs::PointCloud2>("/velodyne/left_front", 1, &velodyne_lidar_color::pointCloudCallback, this); //接收点云数据，进入回调函数pointCloudCallback
		pubCloud=n.advertise<sensor_msgs::PointCloud2>("/velodyne/color_lidar", 1);										   //建立了一个发布器，方便之后发布加入颜色之后的点云
	}																					
